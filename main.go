package main // import "hello-sfnt"
	...
	for _, seg := range segments {
		...
		case sfnt.SegmentOpMoveTo:
			r.MoveTo(
				...
				originX+float32(seg.Args[0].X)/80,
				originY+float32(seg.Args[0].Y)/80,
			)
		...
		case sfnt.SegmentOpQuadTo:
			r.QuadTo(
				originX+float32(seg.Args[0].X)/80,
				originY+float32(seg.Args[0].Y)/80,
				originX+float32(seg.Args[1].X)/80,
				originY+float32(seg.Args[1].Y)/80,
			)
		case sfnt.SegmentOpCubeTo:
			r.CubeTo(
				...
				originX+float32(seg.Args[1].X)/80,
				originY+float32(seg.Args[1].Y)/80,
				originX+float32(seg.Args[2].X)/80,
				originY+float32(seg.Args[2].Y)/80,
			)
		}
	}
